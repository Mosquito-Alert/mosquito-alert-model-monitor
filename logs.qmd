...
if (length(log_files) == 0) {
  ...
  log_table <- log_info %>%
    mutate(
      # Create GitHub links to log files
      github_url = paste0("https://github.com/Mosquito-Alert/mosquito-alert-model-monitor/blob/main/", 
                         gsub("./", "", full_path)),
      filename = sprintf('<a href="%s" target="_blank">%s</a>', github_url, filename),
  ...
  datatable(log_table, 
            ...
            caption = "Available Log Files (click filename to view on GitHub)")
}
```
