...
```{r job-configs, echo=FALSE, message=FALSE, warning=FALSE}
if (nrow(jobs_data) > 0) {
...
```{r resource-analysis, echo=FALSE, message=FALSE, warning=FALSE}
if (nrow(jobs_data) > 0) {
  # Resource usage plot
...
```{r performance-metrics, echo=FALSE, message=FALSE, warning=FALSE}
if (nrow(jobs_data) > 0) {
  # Performance summary
...
```{r scheduling-info, echo=FALSE, message=FALSE, warning=FALSE}
if (nrow(jobs_data) > 0) {
  # Next scheduled runs
...
```{r log-analysis, echo=FALSE, message=FALSE, warning=FALSE}
if (nrow(jobs_data) > 0) {
  # Recent log entries
  log_data <- jobs_data %>%
    select(job_name, latest_log) %>%
    filter(!is.na(latest_log)) %>%
    rename("Job Name" = job_name, "Recent Log Entries" = latest_log)
